...
`queueEnvSchema`, `workerEnvSchema`). Validation happens lazily, inside a
function call (`createEnv(...)`), not at module import time — so importing
a file that uses these schemas never breaks a build that has no real
credentials configured yet (see `apps/web/src/lib/env.ts` for the
pattern).
...
`REDIS_URL` is validated by two separate, narrow schemas rather than the
general-purpose `serverEnvSchema` — most of apps/web never touches Redis, so
requiring it broadly would make unrelated server actions (auth, team
management, …) fail validation for infrastructure they don't use:
